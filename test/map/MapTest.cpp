...
TEST(MapTest, InvalidCreation) {
    ...
    EXPECT_THROW(Map(1, 0, { Tile{ TileType::ROAD, Action::STONE }, Tile{ TileType::BUSH, Action::NONE } }),
          std::invalid_argument);

    EXPECT_THROW(Map(1, 3, { Tile{ TileType::ROAD, Action::STONE }, Tile{ TileType::BUSH, Action::NONE } }),
          std::invalid_argument);
}
...
TEST(MapTest, Indexing) {
    ...
    const Map m1(1, 2, tiles);
    EXPECT_EQ(m1(0, 0), Tile(TileType::ROAD, Action::STONE));
    ...
    EXPECT_EQ(m2(0, 0), Tile(TileType::ROAD, Action::STONE));
